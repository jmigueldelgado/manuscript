...
id_array=id_match$id_jrc %>% paste(collapse=", ")
 ...
 # add this to string
if(file.exists('../data/reservoirs_ts_with_cod.rds')){
    reservoir_ts_with_cod=readRDS('../data/reservoirs_ts_with_cod.rds')
    } else {

select = paste0("SELECT jrc_",project,".id_jrc, ST_area(ST_Transform(jrc_",project,".geom,32629)) as ref_area, ST_Perimeter(ST_Transform(jrc_",project,".geom,32629)) as ref_perimeter, ",project,".area,",project,".ingestion_time,",project,".source_id")

from = paste0("FROM jrc_",project," RIGHT JOIN ",project," ON jrc_",project,".id_jrc=",project,".id_jrc")

where = paste0("WHERE manuscript.id_jrc = ANY(ARRAY[",id_array,"]);")
...
reservoir_ts <- dbGetQuery(con,q_string) %>% as_tibble
dbDisconnect(conn = con)

reservoir_ts_with_cod = reservoir_ts %>% right_join(id_match)

saveRDS(reservoir_ts_with_cod,'../data/reservoirs_ts_with_cod.rds')
}
```
...
#ggplot(ref_dataset) + geom_histogram(aes(x=ref_area))
```
...
```{r}
plt=ggplot(ds)+geom_point(aes(x=volume_obs,y=volume_val,color=id_jrc))
```

The scatter plot is not goot. There is no skill in predicting volume from satellite radar images. This could be expected, since the HAV function is calculated with maximum area and maximum perimeter obtained from a remote sensing dataset. Maybe this could be improved, but most importantly, _can we at least have some skill in predicting approximate states of the system?_ For example, predicting if the reservoir is empty, half-full or full? This would already be a great piece of information.
