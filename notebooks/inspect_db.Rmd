...
ds=inner_join(rs_dataset,validation_dataset,suffix=c('_rs','_val'),by=c('id_jrc','time_stamp')) %>% filter(volume_rs<100000000) %>% filter(volume_rs>0) %>% filter(volume_val>0) %>% mutate(id_jrc=as.factor(id_jrc))
...
ds %>%
  pivot_wider(names_from = algorithm,values_from = volume_rs) %>%
  yardstick::rmse(volume_val,volume_rs)

plt=ggplot(ds)+
  geom_point(aes(x=volume_rs,y=volume_val,color=id_jrc)) +
